...
本记录只把真实运行或自动化测试覆盖的项目标记为已验证。需要用户锁屏、系统睡眠、第二台显示器、真实全屏 Space 或长时间 Instruments 采样的项目继续保持待验证。
...
- [ ] 8 小时 Instruments Energy Log 与内存稳定性采样。

## 7. 阶段退出判断

阶段 0 尚未完成。信号获取、低权限降级、系统睡眠、显示器息屏、真实全屏、外观适配和临时浮层已有直接证据；2026-09-01 的资源批次在 90 分钟时随任务宿主结束，短时指标稳定但不算 8 小时通过。已于 2026-09-04 14:11:57 通过独立用户 LaunchAgent 重启完整采样。多显示器与新账户首次启动已明确延期，不再伪装成已验证。

本机只有一台显示器，因此多显示器实测等待硬件。普通锁屏没有可靠的公开精确信号，已通过删除错误映射并规定未来 idle 延后策略接受该边界。8 小时资源采样完成且无异常后即可评估关闭阶段 0；Stage 1 的纯视觉工作已经并行启动，不等待该采样。
...
1. 等待并审查预计于 2026-09-04 22:12 完成的独立后台 8 小时资源采样；
2. 无异常则记录阶段 0 风险接受决定；Stage 1 视觉原型继续并行推进；
3. 有第二台显示器时补做拓扑实验；
4. 发布前在新账户复测首次启动 TCC 行为；
5. 阶段 2 行为引擎实现 idle 延后与自然休息取消，不依赖精确普通锁屏信号。
